...
local found_entities = find.byClass("prop_vehicle_prisoner_pod", function(ent)
    -- Return nothing (dismiss) if the entity isn't owned by us and its model isn't allowed
...
-- Check whether the entity has been found and if its distance is within limits
if chair and chair:getPos():getDistance(origin) < distance then
    -- Set the chair's color to red and print out a confirmation
