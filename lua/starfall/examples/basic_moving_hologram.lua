...
-- To create a hologram, we make use of the `holograms` library and call its `create` function
-- The function can take up to 4 parameters: `position`, `angle`, `model` and optionally `scale` (not used here)
-- `position`: The `getPos` method will return the entity's world position as a vector object
-- `angle`: The `Angle` constructor will give us an angle object with all axes set to 0, unless specified otherwise
-- `model`: The string describing the path to the model; you can use the spawn menu item's context menu to obtain it, in this case it's a G-Man
...
-- Note how Starfall's documentation only specifies the `Entity.setColor` variant, but omits the `Hologram.setColor` one
...
-- Let's create a function which will be responsible for updating our hologram
local function update_hologram()
    -- Grab the current position of the hologram and add to it a newly constructed vector object with the `X` axis set to 1
...
-- The hologram will get created, its color will be set, and we tell the engine that we want it to rotate, but what about executing the `update_hologram` function?
-- The most common way to update parts of your code is events, or so-called `hooks`.
...
-- There are two main types of timers, ones that execute a certain number of times (or forever), and the `simple` timers that only execute once after a certain delay
...
timer.simple(2, function()
    -- Detach the `update_hologram` function from the `Tick` event by targeting it with its unique name, meaning its `callback` will no longer be executed by this event
    hook.remove("Tick", "UpdateHologram")

    -- Stop rotating the hologram by setting its velocity back to the default 0,0,0
    ...
    timer.create("PlayAnimation", 1.2, 6, function(...)
        -- Tell the game engine to set a non-looping swinging animation to the G-Man's hologram and also play an adequate sound effect
