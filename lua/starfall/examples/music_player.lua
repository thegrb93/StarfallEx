		...
		bass.loadURL(songURL, "3d noblock", function(snd, err, errtxt)
			if isValid(snd) then
				...
				timer.simple( 0.3, function()					
					...
					local tags = station:getTagsID3()		-- ID3v1 only (NOT ID3v2)
					if not tags or not next(tags) then
						tags = station:getTagsOGG()		-- OGG Vorbis comments
					end
					if not tags or not next(tags) then
						tags = station:getTagsMP4()		-- MP4/M4A metadata
					end
					if not tags or not next(tags) then
						tags = station:getTagsWMA()		-- WMA metadata
					end

					-- For Shoutcast/Icecast streams:
					local icyMeta = station:getTagsMeta()		-- ICY metadata (e.g., "Artist - Title")
					local icyHeaders = station:getTagsHTTP()	-- HTTP headers (icy-name, icy-genre, etc.)
					...
					print(string.format("Duration: %.1fs | Bitrate: %d kbps | Sample rate: %d Hz | Bits per sample: %d | Is Block Streamed: %s | Pitch: %d",
						station:getBufferedTime(),
						station:getFileName(),
						station:getAverageBitRate(),
						station:getSamplingRate(),
						station:getBitsPerSample(),
						station:isBlockStreamed(),
						station:getPitch()
					))
				end )
