-- Starfall Derma
-- This is for easily creating derma ui for the Starfall Editor
...
function PANEL:Init()
	...
	function buttonClose:DoClick()
		if frame == SF.Editor.editor then
			SF.Editor.close()
		else
			frame:close()
		end
	...
	if #self.windows ~= 1 then
		...
		function buttonLock:DoClick()
			...
			if frame == SF.Editor.fileViewer then
				SF.Editor.saveSettings()
			end
		end
		buttonHolder:addButton( buttonLock )
		self.buttonLock = buttonLock
	end
end
function PANEL:Think()
	...
	if self.Dragging and not self.locked then
		...
		for k, v in pairs( self.lockChildren ) do
			if not v.locked then goto skip end
		...
		if self == SF.Editor.editor or self == SF.Editor.fileViewer then
			SF.Editor.saveSettings()
		end
	end
	...
	if self.Hovered and self:GetDraggable() and mousey < ( self.y + 24 ) and not self.locked then
...
function PANEL:Init()
	self.RootNode._Think = self.RootNode.Think
	function self.RootNode:Think()
		self:SetPos( 0, 5 )
		self:_Think()
	end
end
...
derma.DefineControl( "StarfallFileBrowser", "", PANEL, "DTree" )
-- End File Browser
