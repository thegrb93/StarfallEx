...
Runs a new http POST request";["fname"]="post";["library"]="http";["name"]="http_library.post";["param"]={[1]="url";[2]="payload";[3]="callbackSuccess";[4]="callbackFail";[5]="headers";["callbackFail"]="the function to be called on request fail, taking the failing reason as an argument";["callbackSuccess"]="the function to be called on request success, taking the arguments body (string), length (number), headers (table) and code (number)";["headers"]="POST headers to be sent";["payload"]="POST payload to be sent, can be both table and string. When table is used, the request body is encoded as application/x-www-form-urlencoded";["url"]="http target url";};["private"]=false;["realm"]="sh";["summary"]="\
