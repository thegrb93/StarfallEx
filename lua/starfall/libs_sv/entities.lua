...
function ents_methods:applyDamage(amt, attacker, inflictor, dmgtype, pos)
	...
	if attacker~=nil then
		dmg:SetAttacker(eunwrap(attacker))
	else
		dmg:SetAttacker(instance.player)
	end
	if inflictor~=nil then
		dmg:SetInflictor(eunwrap(inflictor))
	end
	if dmgtype~=nil then
...
-- - starfall_prop
-- - starfall_processor
-- @param function? startTouchCB Optional StartTouch callback function. With the argument:
-- 1. Entity object: the object entering our entity's bounds.
-- @param function? endTouchCB Optional EndTouch callback function. With the argument:
