-- It's LuaXml with modifications
local base = _G
module("xml")
...
function new(arg)
  if base.type(arg)=="table" then 
    base.setmetatable(arg,{__index=_M, __tostring=str})
	return arg
  end
  local var={}
  base.setmetatable(var,{__index=_M, __tostring=str})
...
function str(var,indent,tagValue)
  ...
  local indentStr=base.string.rep("\t",indent)
  local tableStr=""
  
  if base.type(var)=="table" then
    local tag = var[TAG] or tagValue or base.type(var)
