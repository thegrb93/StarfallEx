...
function SF_Parser:StmtFor()
	if self:AcceptRoamingToken("for") then
		...
		-- for[(]Var = Start, Stop[, Step][)] <Block>
		-- OR
		-- for[(]Key, Val in List[)] <Block>
		...
		if !self:AcceptRoamingToken("var") then
			self:Error("Variable expected for the index")
		end
		...
		if self:AcceptRoamingToken("com") then
			...
			local var2 = self:GetTokenData()
			
			if not self:AcceptRoamingToken("in") then
				self:Error("In keyword (in) expected")
			end
		...
		elseif self:AcceptRoamingToken("ass") then
			...
			self:Error("Assignment operator (=) or comma (,) expected to preceed variable")
		end
...
function SF_Parser:StmtBreak()
	...
	return self:StmtIncDec()
end

function SF_Parser:StmtIncDec()
	if self:AcceptRoamingToken("var") then
		local trace = self:GetTokenTrace()
		local var = self:GetTokenData()
	...
	if self:AcceptRoamingToken("var") then
		...
		if self:AcceptTailingToken("lsb") then
			...
			elseif self:AcceptRoamingToken("rsb") then
				if self:AcceptRoamingToken("ass") then
					return self:Instruction(trace, "set", var, expr, self:StmtIndex())
				end
			else
		...
		elseif self:AcceptRoamingToken("ass") then
			return self:Instruction(trace, "ass", var, self:StmtIndex())
		end
...
function SF_Parser:Cond()
	...
	if no_pa then
		if self:AcceptRoamingToken("blk") or self:AcceptRoamingToken("lcb") then
			self:TrackBack()
		else
			self:Error("Block beginning missing to close condition")
		end
	elseif not self:AcceptRoamingToken("rpa") then
		self:Error("Right parenthesis ()) missing, to close condition")
	end
...
function SF_Parser:Block(block_name, no_start)
	local trace = self:GetTokenTrace()
	local stmts = self:Instruction(trace, "seq")
	...
	if self:AcceptRoamingToken("lcb") then
		ending = "rcb"
	elseif no_start or self:AcceptRoamingToken("blk") then
		ending = "end"
	else
		--self:Error("Left curly bracket ({) must appear after "..(block_name or "condition"))
	...
	if self:AcceptRoamingToken(ending) then
		return stmts
	end
	...
	if self:HasTokens() then
		while true do
			if self:AcceptRoamingToken("com") then
				self:Error("Statement separator (,) must not appear multiple times")
			elseif self:AcceptRoamingToken(ending) then
				-- TODO: Error on this or just ignore?
	...
	if ending == "end" then
		self:Error("End (end) missing, to close statement block", token)
	else
